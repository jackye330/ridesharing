    ...
    for vehicle in vehicles:
        if vehicle not in payments:
            ...
            if vehicle.status == Vehicle.WITHOUT_MISSION_STATUS:
                vehicle.drive_on_random(row_size, col_size)
            else:
                vehicle.drive_on_route_plan()
            continue
...
def dispatch_orders_with_iterative(
    ...
    for vehicle in vehicles:
        if vehicle not in payments:
            # 没有在胜者集合的而且没有路线目标的随机更新下一个位置
            if vehicle.status == Vehicle.WITHOUT_MISSION_STATUS:
                vehicle.drive_on_random(shortest_distance, shortest_path_with_minute)
            else:
                vehicle.drive_on_route_plan(shortest_distance, shortest_path)
            continue
        # 如果是胜者者的话就要决定最优路线而且还要更新订单的状态
        order, payment = payments[vehicle]
