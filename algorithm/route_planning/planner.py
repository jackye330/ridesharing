...
class InsertingPlanner(RoutePlanner):
    ...
    """
    __slots__ = ["optimal_route_distance", "optimal_i", "optimal_j"]

    def reset(self):
        super(InsertingPlanner, self).reset()
        self.optimal_route_distance = POS_INF
        self.optimal_i = -1
        self.optimal_j = -1
    ...
    def planning2(self, vehicle_type: VehicleType, old_route: List[OrderLocation], order: Order, current_time:int, network):
        ...
        def _recursion(pre_location: GeoLocation, k1: int, k2: int, seats: int, drive_dists: float, result: List[int]):
            if k1 == len(old_route) and k2 == 2:
                if drive_dists - vehicle_type.service_driven_distance < self.optimal_route_distance:
                    self.optimal_route_distance = drive_dists - vehicle_type.service_driven_distance
                    self.optimal_i = result[0]
                    self.optimal_j = result[1]
            elif is_enough_small((drive_dists - vehicle_type.service_driven_distance), self.optimal_route_distance):
                ...
                for n_k1, n_k2, o_loc in locations:
                    ...
                    arr = drive_dists + network.get_shortest_distance(pre_location, o_loc)
        ...
        def _recursion(_cur_loc_list: List[OrderLocation], _rem_loc_list: List[OrderLocation], cur_seats: int, cur_dists: float):
            ...
            if len(_rem_loc_list) == INT_ZERO:
                ...
                for i, o_loc in enumerate(_rem_loc_list):
                    ...
                    if isinstance(o_loc, PickLocation):
                        upper_time = (belong_o.request_time + belong_o.wait_time - current_time)
                        if cur_seats - belong_o.n_riders >= INT_ZERO and network.is_smaller_bound_distance(_cur_dists - old_dists, avg_speed * upper_time):
                            # 人数满足要求且不超过最大等待时间
                            _rem_list_copy = _rem_loc_list[:i] + _rem_loc_list[i + 1:]
                            _rem_list_copy.append(belong_o.drop_location)
                            pick_up_dists_dict[belong_o] = _cur_dists
                            for n_r, n_i in _recursion(_cur_loc_copy, _rem_list_copy, cur_seats - belong_o.n_riders, _cur_dists):
                                yield n_r, n_i
                            pick_up_dists_dict.pop(belong_o)

                    else:
                        real_detour_dist = _cur_dists - (pick_up_dists_dict[belong_o] if belong_o in pick_up_dists_dict else belong_o.pick_up_distance) - belong_o.order_distance
