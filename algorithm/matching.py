            ...
            if vehicle.is_between == Vehicle.IS_BETWEEN_TWO_INDEX:
                ...
                if shortest_distance[vehicle.goal_index, order.start_location.osm_index] + \
                    ...
                    if two_location_distance + vehicle.between_distance > rest_of_time * Vehicle.AVERAGE_SPEED:
                        continue
            if vehicle.is_between == Vehicle.NOT_BETWEEN_TWO_INDEX \
                    and two_location_distance > rest_of_time * Vehicle.AVERAGE_SPEED:
                continue
...
def update_vehicle_location(shortest_distance: np.ndarray, shortest_path: np.ndarray, shortest_path_with_minute: np.ndarray,
                            adjacent_nodes: np.ndarray, vehicles: Set[Vehicle],
                            payments: Dict[Vehicle, Tuple[Order, float]], current_time: int):
    """
    :param shortest_distance: 最短路径长度矩阵
...
def orders_matching_with_gm(bids: Dict[Vehicle, Dict[Order, float]]) \
    ...
    for vehicle in matched_vehicles:
        ...
        feasible_vehicles.remove(vehicle)   # 移除了有什么用？
...
def orders_matching_with_nearest_matching(shortest_distance: np.ndarray, orders: Set[Order],
    ...
    for order in orders:
        ...
        for vehicle in vehicles:
            # print(order.order_id, distance[vehicle])
            # 如果人数过多就继续考虑
            if order.n_riders > vehicle.n_seats:
                continue
            ...
            if 0.0 < additional_cost < order.trip_fare:
                ...
                matched_orders.add(order)
                matched_vehicles.add(vehicle)
                # vehicle.n_seats -= order.n_riders   # ??减多了
