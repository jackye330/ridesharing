...
def order_matching_simulation(shortest_distance, shortest_path, shortest_path_with_minute, orders, vehicles, method):
    ...
    over_distance_order_per_min = []    # 距离太远接不到的订单
    ...
    for t in range(MIN_REQUEST_TIME, MAX_REQUEST_TIME):
        ...
        # 激活预留车辆
        if t == 999:
            for i in range(3000, len(vehicles)):    # 总车辆数len，初始激活树木3000
                vehicles[i].activated = Vehicle.POSITIVE
        ...
        empty_vehicle_num = 0
        total_vehicle_num = len(vehicles) - 500
        if t >= 900:
            total_vehicle_num = len(vehicles)
        for vehicle in vehicles:
            # 统计空闲车的数量
            if vehicle.activated == Vehicle.POSITIVE and vehicle.status == Vehicle.WITHOUT_MISSION_STATUS:
                empty_vehicle_num += 1
        empty_vehicle_rate.append(empty_vehicle_num / total_vehicle_num)
        print(t)
...
if __name__ == '__main__':
    ...
    for k in range(0, 1):
        ...
        with open("./env_data/vehicle/{0}_{1}.pkl".format(VEHICLE_NUMBER, k), "rb") as file:
            ...
            print(len(vehicles))
            #   设置未激活车辆
            for i in range(len(vehicles) - negative_vehicle, len(vehicles)):
                vehicles[i].activated = Vehicle.NEGATIVE
        ...
        with open("./result/per_min/{0}/{1}_{2}_{3}.pkl".format(cur_model, VEHICLE_NUMBER, k, "dynamic_500"), "wb") as file:
