...
from algorithm.order_dispatching import *
from algorithm.order_matching import *
from setting import *
from utility import *


def order_matching_simulation(shortest_distance, shortest_path, shortest_path_with_minute, adjacent_nodes, orders,
                              vehicles, method):
    ...
    for t in range(MIN_REQUEST_TIME, MAX_REQUEST_TIME):
        ...
        # 车辆更新位置
        update_vehicle_location(shortest_distance, shortest_path, shortest_path_with_minute, adjacent_nodes, vehicles,
                                payments, t)
        for vehicle in vehicles:
...
def order_dispatch_simulation(shortest_distance, shortest_path, shortest_path_with_minute, adjacent_nodes,
    ...
    for t in range(MIN_REQUEST_TIME, MAX_REQUEST_TIME):
        ...
        dispatch_result = method(shortest_distance, shortest_path, shortest_path_with_minute, adjacent_nodes,
                                 un_dispatched_orders, vehicles, t)
        # 记录结果
    ...
    for t in range(MIN_REQUEST_TIME, MAX_REQUEST_TIME):
        ...
        for order in order_t:
            ...
            for vehicle in vehicles:  # 计算每个车的投标
                ...
                old_profit, old_cost, _ = vehicle.find_best_schedule([], rem_list.copy(), shortest_distance, t, 0.0,
                                                                     0.0, [])
                rem_list.append(order.start_location)
                # 把这里算的路径存起来
                new_profit, new_cost, new_route_plan = vehicle.find_best_schedule([], rem_list.copy(),
                                                                                  shortest_distance, t, 0.0, 0.0, [])
